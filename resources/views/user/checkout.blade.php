...
<div class="container mb-5 pb-3">
      <div class="bg-light box-shadow-lg rounded-lg overflow-hidden">
        <div class="row">
          ...
          <form action="{{ route('procheckout') }}" class="needs-validation" id="checkout_form" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
             ...
             <div class="widget mb-3 d-lg-none">
                ...
                <div class="media align-items-center pb-2 border-bottom">
                        ...
                        <div class="media-body pl-1">
                            <h6 class="widget-product-title text-left"><a href="{{ url('/item') }}/{{ $itemArray[0]->item_slug }}">{{ $cart->item_name }}</a></h6>
                            <div class="widget-product-meta"><span class="text-accent border-right pr-2 mr-2">{{ $allsettings->site_currency_symbol }} {{ $cart->item_price }}</span>
                              <span class="font-size-xs text-muted">{{ $cart->license }}@if($cart->license == 'regular') ({{ Helper::translation(2890,$translate) }}) @elseif($cart->license == 'extended') ({{ Helper::translation(2891,$translate) }}) @endif</span>
                            </div>
                        </div>
                </div>
              ...
              // echo $total_prices;
                ...
                <input type="hidden" name="total_prices" value="{{ base64_encode(rtrim($total_prices,',')) }}">
                  ...
                  @if($payment == 'paypal')
                        <div class="card-body font-size-sm custom-control">
                          <div class="custom-radio">
                            <span class='font-weight-medium'><input id="opt1-{{ $payment }}" name="payment_method" type="radio" class="custom_radio" value="{{ $payment }}" @if($no == 1) checked @endif data-bvalidator="required"></span> {{ Helper::translation(5937,$translate) }} - {{ Helper::translation(4905,$translate) }}
                          </div>
            ...
            <div class="cz-sidebar-static h-100 ml-auto border-left">
              <div class="widget mb-3">
                ...
                @endif
                  ...
                  <div class="media-body pl-1">
                    <h6 class="widget-product-title"><a href="{{ url('/item') }}/{{ $itemArray[0]->item_slug }}">{{ $cart->item_name }}</a></h6>
                    <div class="widget-product-meta"><span class="text-accent border-right pr-2 mr-2">{{ $allsettings->site_currency_symbol }} {{ $cart->total_price }}</span><span class="font-size-xs text-muted">{{ $cart->license }}@if($cart->license == 'regular') ({{ Helper::translation(2890,$translate) }}) @elseif($cart->license == 'extended') ({{ Helper::translation(2891,$translate) }}) @endif</span></div>
                  </div>
                </div>
                @php 
                $subtotal += $cart->total_price;
        ...
        if($item_user_type == 1)
                ...
                @if($allsettings->site_extra_fee != 0)
                  ...
                  $priceamount = $new_price;
                  @endphp
                  <li class="d-flex justify-content-between align-items-center"><span class="mr-2">{{ Helper::translation(2895,$translate) }}</span><span class="text-right">{{ $coupon_discount }} 
                    {{ $allsettings->site_currency }}</span></li>
                  @else
                  @php 
                  $final = $subtotal+$allsettings->site_extra_fee; 
                  $last_price =  $subtotal;
