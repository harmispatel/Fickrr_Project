					...
					@endif
						<div class="navbar-toolbar d-flex align-items-center order-lg-3">
							<button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarCollapse"><span class="navbar-toggler-icon"></span></button><a class="navbar-tool d-none d-lg-flex" href="#searchBox" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="searchBox"><span class="navbar-tool-tooltip">{{ Helper::translation(4857,$translate) }}</span>
								...
								<a class="navbar-tool-icon-box border dropdown-toggle" @if(Auth::user()->id == 1) href="{{ url('/admin') }}" target="_blank" @else href="{{ URL::to('/user') }}/{{ Auth::user()->username }}" @endif>        
							...
							<div class="navbar-tool dropdown ml-3">
								<a class="navbar-tool-icon-box bg-secondary dropdown-toggle" href="{{ url('/procart') }}"><span class="navbar-tool-label">{{ $cartcount }}</span><i class="navbar-tool-icon dwg-cart"></i></a>
								...
								<div class="dropdown-menu dropdown-menu-right" style="width: 20rem;">
									<div class="widget widget-cart px-3 pt-2 pb-3">
										...
										<div class="d-flex flex-wrap justify-content-between align-items-center py-3">
											<div class="font-size-sm mr-2 py-2"><span class="text-muted">{{ Helper::translation(2896,$translate) }}:</span><span class="text-accent font-size-base ml-1">{{ $allsettings->site_currency_symbol }} {{ $subtotall }}</span>
											</div>
											<a class="btn btn-outline-secondary btn-sm" href="{{ url('/cart') }}">{{ Helper::translation(3021,$translate) }}<i class="dwg-arrow-right ml-1 mr-n1"></i></a></div><a class="btn btn-primary btn-sm btn-block" href="{{ url('/checkout') }}"><i class="dwg-card mr-2 font-size-base align-middle"></i>{{ Helper::translation(2899,$translate) }}</a>
									</div>
								</div>
								@endif
							</div>
						...
						<div class="collapse navbar-collapse mr-auto order-lg-2" id="navbarCollapse">
							...
							<ul class="navbar-nav">
								...
								<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="{{ url('/shop') }}">{{ Helper::translation(3178,$translate) }}</a>
									<ul class="dropdown-menu">
										<li><a class="dropdown-item" href="{{ url('/') }}/featured-items">{{ Helper::translation(3027,$translate) }}</a></li>
										<li><a class="dropdown-item" href="{{ url('/free-items') }}">{{ Helper::translation(3016,$translate) }}</a></li>
										<li><a class="dropdown-item" href="{{ url('/') }}/new-releases">{{ Helper::translation(4842,$translate) }}</a></li>
										<li><a class="dropdown-item" href="{{ url('/') }}/popular-items">{{ Helper::translation(3181,$translate) }}</a></li>
									 </ul>
								</li>
								<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="javascript:void(0)" data-toggle="dropdown">{{ Helper::translation(2879,$translate) }}</a>
									<ul class="dropdown-menu">
									 @foreach($categories['menu'] as $menu)
										<li class="dropdown"><a class="dropdown-item dropdown-toggle" href="{{ URL::to('/shop/category/') }}/{{$menu->category_slug}}">{{ $menu->category_name }}</a>
											<ul class="dropdown-menu">
								...
								<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="javascript:void(0)" data-toggle="dropdown">{{ Helper::translation(3029,$translate) }}</a>
									<ul class="dropdown-menu">
										@foreach($allpages['pages'] as $pages)
										<li><a class="dropdown-item" href="{{ URL::to('/page/') }}/{{ $pages->page_slug }}">{{ $pages->page_title }}</a></li>
										@endforeach
									</ul>
								</li>
								@endif
									<li class="nav-item dropdown"><a class="nav-link" href="{{ url('/top-authors') }}">{{ Helper::translation(3028,$translate) }}</a></li>
									<li class="nav-item dropdown"><a class="nav-link" href="{{ URL::to('/flash-sale') }}">{{ Helper::translation(2993,$translate) }}</a></li>
							 </ul>
						</div>
					</div>
				</div>
