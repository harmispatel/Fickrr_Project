...
class Items extends Model
{
  ...
  public static function  updateitemLocationData($data,$item_id){
  ...
  public static function updateitemHotelData($data,$item_id){

  DB::table('item_hotel')->where('hotel_id', '=', $item_id)->delete(); 

      foreach($data as $key=>$value){



      DB::table('item_hotel')->insert( array(
            'hotel_id'    =>  $value,
            'item_id'   =>   $item_id
      ));
    ...
    public static function  updateRoomtyp($data,$item_id){
        DB::table('item_room')->where('item_id', '=', $item_id)->delete(); 
        foreach($data as $key=>$value){
              DB::table('item_room')->insert( array(
                    'item_id'    =>  $item_id, 
   ...
   public static function  updatemanufacttyp($data,$item_id){


        DB::table('item_manfct')->where('item_id', '=', $item_id)->delete(); 


        foreach($data as $key=>$value){
              DB::table('item_manfct')->insert( array(
                    'item_id'    =>  $item_id, 
                    'item_manfct_id'   =>   $value
              ));
  ...
  public static function getItemManufactureData($item_id){
 return DB::table('item_manfct')->where('item_id', '=', $item_id)->pluck('item_manfct_id')->toArray();; 
  }

  public static function getLocationData($item_id){
 return DB::table('item_room')->where('item_id', '=', $item_id)->orderBy('item_loccate_id','asc')->pluck('item_loccate_id')->toArray();; 
  }

   public static function getRoomImagesData($item_tockend){


        return DB::table('items_roomimages')->where('item_token', '=',$item_tockend)->get();; 
   ...
   public static function getLocationDatawithName($item_id){
      return DB::table('category')->whereIn('cat_id', $item_id)->get();; 
  }
  ...
  //  echo "<pre>";print_r( Auth::user());exit;
    ...
    // $value=DB::table('items')->join('users','users.id','items.user_id');

    // if($user_id > 0){

    //     $value= $value->where('items.user_id','=',$user_id);


    // }
    // $value= $value->where('items.item_status','!=',2)->where('items.drop_status','=','no')->orderBy('items.item_id', 'desc')->get(); 



      $value=Items::with('hasOneUser')->where('items.drop_status','=','no');
    $value= $value->where(function( $value){
            $value->where('item_type','=','hotel')->orWhere('item_type','=','restaurant')->orWhere('item_type','=','spa');
    });
    //   if($user_id > 0){
    ...
    // }
     $value= $value->orderBy('items.item_id', 'desc')->get(); 
  ...
  public static function getItemTypeData($data){

    return DB::table('item_type')->where('item_type_name','=',$data)->first();
  }
  ...
  public static function getentireItem()
  {
    ...
    // $value=DB::table('items')->join('users','items.user_id', '=','users.id')->where('items.drop_status','=','no');
    // $value= $value->where(function( $value){
    //         $value->where('item_type','=','hotel')->orWhere('item_type','=','restaurant');
    // })->where('item_id','=','106')->orderBy('items.item_id', 'desc')->get(); 
  ...
  public static function saveitemImages($imgdata)
  {
   ...
   public static function saveroomImages($imgdata,$item_tocken)
  {
        DB::table('items_roomimages')->where('item_token', '=', $item_tocken)->delete(); 


        foreach($imgdata as $key=>$value){

             DB::table('items_roomimages')->insert( array(
       ...
       // DB::table('items_roomimages')->insert($imgdata);
   ...
   public static function getgroupitemData()
  {
   ...
   public function Ratings()
    ...
    //return $this->hasOne(Users::class,'id', 'user_id');
     ...
     public function hasManysItemroom(){

    //return $this->hasOne(Users::class,'id', 'user_id');
       return $this->hasMany('Fickrr\Models\Itemroom', 'item_id', 'item_id');
    }

     public function hasOneDesignershop(){

    //return $this->hasOne(Users::class,'id', 'user_id');
       return $this->hasMany('Fickrr\Models\Designershops','shopitem_id', 'item_id');
    }

     public function hasManysItemhotel(){

    //return $this->hasOne(Users::class,'id', 'user_id');
       return $this->hasMany('Fickrr\Models\Itemhotel', 'hotel_id', 'item_id');
    }
  ...
  public static function singleitemData($item_slug){
      ...
      $value=Items::with('hasOneUser','hasManysItemroom')->where('items.item_status','=',1)->where('items.drop_status','=','no')->where('items.item_slug','=',$item_slug)->first(); 
      //$value=DB::table('items')->where('items.item_status','=',1)->where('items.drop_status','=','no')->where('items.item_slug','=',$item_slug)->first(); 
  ...
  public static function updatefavouriteData($item_id,$record){
        DB::table('items')->where('item_id', $item_id)->update($record);
  }
  ...
  public static function viewItemtypeName($type_name)
  {
    $value = DB::table('item_type')
