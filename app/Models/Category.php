...
class Category extends Model
{
  ...
  public static function allsubcategoryData()
  {

        $value=DB::table('subcategory')->where('drop_status','=','no')->where('subcategory_status','=','1');
         $value= $value->where(function($q){
            $q->where('subcatparent_id','=',null)->orWhere('subcatparent_id','=',0);
         });
        $value= $value->orderBy('cat_id', 'desc')->get(); 
        // $value= $value->where('subcatparent_id ','=',null)->orderBy('cat_id', 'desc')->get(); 
        return $value;
