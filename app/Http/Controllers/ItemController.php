...
class ItemController extends Controller
{
	...
	public function view_favorite_item($itemid,$favorite,$liked = 0)
	{  
	   ...
	   $getcount  = Products::getfavouriteCount($item_id,$log_user);
	   if($getcount == 0)
	   {
	      $data = array ('item_id' => $item_id, 'user_id' => $log_user);
		  Products::savefavouriteData($data);
		  $record = array('item_liked' => $like);
		  Products::updatefavouriteData($item_id,$record);
    ...
    // // }
    //  $value= $value->orderBy('items.item_id', 'desc')->paginate(5); 
