...
class SettingsController extends Controller
{
	...
	public function update_general_settings(Request $request)
	{
		 ...
		 $image_size = $allsettings->site_max_image_size;
		  ...
		  $calltoactiontext = $request->input('calltoactiontext');

		  $staff_fav_content = $request->input('staff_fav_content');
		  $save_stafffav_image = $request->input('save_stafffav_image');
		...
		if ($request->hasFile('site_favicon')) {
		  ...
		  if ($request->hasFile('site_banner')) {
			...
			$imagearray = $request->file('site_banner');
			 	$imagename=array();
		   	...
		   	$banner_image =implode(",",$imagename);
			// $img_name = time() . '11.'.$image->getClientOriginalExtension();
			// $destinationPath = public_path('/storage/settings');
			// $imagePath = $destinationPath. "/".  $img_name;
			// $image->move($destinationPath, $img_name);
		  ...
		  else
		  {
		   ...
		   if ($request->hasFile('stafffav_image')) {
		     
			Settings::dropStafffavImage($sid); 
		   
			$image = $request->file('stafffav_image');
			$img_name = time() . '6713.'.$image->getClientOriginalExtension();
			$destinationPath = public_path('/storage/settings');
			$imagePath = $destinationPath. "/".  $img_name;
			$image->move($destinationPath, $img_name);
			$stafffav_image = $img_name;
		  }
		  else
		  {
		     $stafffav_image = $save_stafffav_image;
		  }
  ...
  public function edit_country($cid)
	...
	public function delete_general_settings_watermark(Request $request){
		 ...
		 if($type == 'watermark'){
		 	...
		 	$data = array('site_watermark' => '', 'watermark_option' => 0);   
		 	Settings::updategeneralData($cid, $data);
		 	...
		 	$banner=$request->input('banner');
		 	 ...
		 	 $bannerarray=explode(",",$banner);
			 $ban=array();
			 if(count($bannerarray) > 0 && $bannerarray[0] != ''){
			...
			if(count($ban) > 0){
				$data=array('site_banner'=>implode(",",$ban));	
			}else{
				$data=array('site_banner'=>'');	
			}
