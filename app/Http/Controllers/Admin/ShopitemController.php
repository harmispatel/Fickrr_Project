...
class ShopitemController extends Controller
{
	...
	public function view_items()
	{
		  ...
		  $itemData['item'] = Items::getProductItem();
	  ...
	  return view('admin.shop.shopitems')->with($data);
	...
	public function edit_item($token)
	{
		...
		//echo $edit['item']->item_id;
		//$itemroom=Items::with('hasManysItemroom')->where('item_id',$edit['item']->item_id)->pluck('item_loccate_id')->toArray();
		$itemroom=Itemroom::with('hasOneItems')->where('item_id',$edit['item']->item_id)->pluck('item_loccate_id')->toArray();
		
		$itemhotel=Itemhotel::with('hasOneItems')->where('item_id',$edit['item']->item_id)->pluck('hotel_id')->toArray();
        ...
        $cat_id = @$edit['item']->item_category;
     	...
     	//  $item_category= Items::getLocationData($edit['item']->item_id);
		...
		$data = array('edit' => $edit,'token' => $token, 'item_image' => $item_image, 'getcount' => $getcount, 'cat_id' => $cat_id, 'cat_name' => $cat_name,  'getvendor' => $getvendor, 'type_name' => $type_name,'attri_field' => $attri_field, 'attribute' => $attribute, 'typer_id' => $typer_id,'item_room'=>$item_room,'itemWell'=>$itemWell,'itemroom'=>$itemroom,'itemhotel'=>$itemhotel);
	  
	   return view('admin.edit-shopitem')->with($data);
    ...
    public function upload_item($itemtype)
    {
		...
		$getvendor= Members::getvendorData();
		...
		// 	$q=Items::with('hasManysItemroom')->where('user_id','=',$user_id);
		// $q=$q->where(function($q){
		// 	$q->where('item_type','=','hotel')->orWhere('item_type','=','restaurant');
		// })->get();
		...
		$data = array('getvendor' => $getvendor, 'itemWell' => $itemWell, 'attribute' => $attribute, 'type_id' => $type_id, 'type_name' => $type_name,'categories'=>$categories,'shops'=>$q);
        return view('admin.upload-shopitem')->with($data);
    }
    ...
    public function fetch_vendor_hotel_room(Request $request){
    	...
    	$sel_hotel=explode(",",$request->sel_hotel);
    	$sel_room=explode(",",$request->sel_room);
		...
		$hotel_html='';$room_html='';$html='';
		  ...
		  $hotel_html.='<div class="form-group" id="removehhtml">';
		  $hotel_html.='<label  for="site_title" class="control-label mb-1">Hotels</label>';
		  $room_html.='<div class="form-group" id="removerhtml">';
		   $room_html.='<label  for="site_title" class="control-label mb-1">Roomtype</label>';
		if(count($q) > 0){
			$hotel_html.='<select name="hotelselection[]" id="hotelselection"  multiple="multiple" class="form-control">';
 ...
 $hotel_html.='<option value="'.$value->item_id.'" '.$sel.'>'.$value->item_name.'</option>';
    ...
    $hotel_location=$request->hotelselection;
    $room_type=array();
	 ...
	 //  echo "<pre>";print_r($request->all());exit;
	   ...
	   else
	   {
		...
		$apiKey = 'AIzaSyDXb_Mq0lK0KfNT-1l4NxdUEHDNmIcPmFE'; // Google maps now requires an API key.
			...
			$cat_id='';
			$item_type='product';
		    $data = array('item_name' => $item_name, 'item_desc' => $item_desc, 'item_thumbnail' => $item_thumbnail, 'item_preview' => $item_preview, 'item_file' => $item_file, 'item_category' =>$cat_id, 'item_category_type' => $cat_name, 'item_type' => $item_type, 'regular_price' => $regular_price, 'extended_price' => $extended_price, 'demo_url' => $demo_url, 'item_tags' => $item_tags, 'item_status' => $item_status, 'item_shortdesc' => $item_shortdesc, 'free_download' => $free_download, 'item_slug' => $item_slug, 'video_url' => $video_url,  'updated_item' => $updated_item, 'item_flash' => $item_flash, 'video_preview_type' => $video_preview_type, 'video_file' => $video_file, 'item_type_cat_id' => $item_category,'latitude'=>$latitude,'longitude'=>$longitude,'address'=>$address);
			 // echo "Hsfsdfhh";
		    Items::updateitemData($item_token,$data);
		    ...
		    if(count($room_type) > 0 && $room_type[0] != ''){
		    	...
		    	//$itemsIdd=$request->input('item_id');
		    	 Items::updateitemLocationData($room_type,$itemdd_id);
		    }
		   ...
		   // $itemsIdd= Items::saveitemData($data);
		    if(count($hotel_location) > 0 && $hotel_location[0] != ''){
		    	...
		    	//$item_id=$request->input('item_id');
		    	 Items::updateitemHotelData($hotel_location,$itemdd_id);
		    }
		...
		//  echo "Hhh";exit;
           ...
           // echo "TESRessdfsdfsdftREstT ites";exit;
			...
			return redirect('/admin/shopItems')->with('success', $item_approve_status);
	...
	public function save_items(Request $request){
	  // echo "<pre>";print_r($request->all());exit;
	   ...
	   $address =$request->input('location');;
		    ...
		    $data = array('user_id' => $user_id, 'item_token' => $item_token, 'item_name' => $item_name, 'item_desc' => $item_desc, 'item_thumbnail' => $item_thumbnail, 'item_preview' => $item_preview, 'item_file' => $item_file, 'item_category' =>$cat_id, 'item_category_type' => $cat_name, 'item_type' => $item_type, 'regular_price' => $regular_price, 'extended_price' => $extended_price, 'demo_url' => $demo_url, 'item_tags' => $item_tags, 'item_status' => $item_status, 'item_shortdesc' => $item_shortdesc, 'free_download' => $free_download, 'item_slug' => $item_slug, 'video_url' => $video_url, 'item_stafffav' => 'yes', 'tiem_ravybeexc' => '0', 'created_item' => $created_item, 'updated_item' => $updated_item, 'video_preview_type' => $video_preview_type, 'video_file' => $video_file, 'item_type_cat_id' => $item_category,'latitude'=>$latitude,'longitude'=>$longitude,'address'=>$address);
		    ...
		    if(count($room_type) > 0 && $room_type[0] != ''){
		    	...
		    	//$item_id=$request->input('item_id');
		    	 Items::updateitemLocationData($room_type,$itemsIdd);
		    }
