...
class ProductController extends Controller
{
	...
	public function edit_item($token)
	{
	   ...
	   { 
	   return view('user.edit-item')->with($data);
	   }
	   else
	   {
	    return view('404');
	...
	public function manage_item() // it is hotel/Restuarant/spa  page
	{
	 
	  //echo "hii TEST ReST REST ";exit;
	...
	//  echo "<pre>";print_r($itemData['item'] );exit;
	  ...
	  return view('user.manage-item',[ 'itemData' => $itemData, 'encrypter' => $encrypter, 'viewitem' => $viewitem]);
	...
	//   $free_download = $request->input('free_download');
	      ...
	      $facebook=$request->input('socialmedialink');
	  ...
	  $rules = array(
		...
		else
		{
		 ...
		 $updated_item = date('Y-m-d H:i:s'); 
		    ...
		    $data = array('item_name' => $item_name, 'item_desc' => $item_desc, 'item_preview' => $item_preview, 'item_category' =>$cat_id, 'item_category_type' => $cat_name, 'item_type' => $item_type,'demo_url' => $demo_url, 'item_tags' => $item_tags, 'item_status' => $item_status, 'item_shortdesc' => $item_shortdesc, 'item_slug' => $item_slug, 'video_url' => $video_url, 'updated_item' => $updated_item, 'video_preview_type' => $video_preview_type, 'video_file' => $video_file, 'item_type_cat_id' => $item_category,'facebook'=>$facebook,'instagram'=>$instagram,'linkedin'=>$linkedin,'website'=>$website,'phonenumber'=>$phonenumber,'email'=>$email);
	...
	public function view_cart(Request $request)
	{
	 ...
	 //echo "<pre>";print_r($request->all());exit;
	  ...
	  $vendorstore = $request->input('vendorstoreid');
	  $item_name = $request->input('item_name');
	  ...
	  // echo ($price * $item_quantity);exit;
	   $getcount  = Products::getorderCount($item_id,$user_id,$order_status);
	   	
	//  echo "<pre>";print_r($getcount);
	   $savedata = array('user_id' => $user_id, 'pro_id' => $item_id,'vendorstoreid'=>$vendorstore, 'item_name' => $item_name, 'item_user_id' => $item_user_id, 'item_token' => $item_token, 'license' => $license, 'start_date' => $start_date, 'end_date' => $end_date, 'item_price' => $price, 'vendor_amount' => $vendor_amount, 'admin_amount' => $admin_amount, 'total_price' => ($price * $item_quantity), 'order_status' => $order_status);
	   $savedata['total_price']= ($price * $item_quantity);
	   $savedata['item_qunatity']= $item_quantity;

	// echo "<pre>";print_r($savedata);
	   //print_r(array_splice($savedata,1,1));
	 
	   //print_r($savedata);
	// exit;
	   
	   $updatedata = array('license' => $license, 'start_date' => $start_date, 'end_date' => $end_date, 'item_price' => $price, 'total_price' => ($price * $item_quantity),'item_qunatity'=>$item_quantity);
	   
	   if($getcount == 0){


	     	$orderId= Products::savecartData($savedata);
	    	// echo  $orderId;exit;
		    // if($orderId > 0){
		    ...
		    //  	//array_pop($savedata);
		    //  	//unset($savedata
		   	// 	Productsuborder::savesubordercartData($savedata);

		    // }
		  	return redirect('procart')->with('success','Item has been added to cart'); 
	   }
	   else{


	      	Products::updatecartData($item_id,$user_id,$order_status,$updatedata);
		  	return redirect('procart')->with('success','Item has been updated to cart'); 
	   }
	...
	public function show_checkout()
	{
	 	...
	 	$cart['item'] = Products::getcartData();
		// echo "<pre>";print_r( $cart['payment']);exit;
	  	//$cart_count = Items::getcartCount();
		$cart_count = count( $cart['item'] );
		$mobile['item'] = Products::getcartData();

	 if($cart['item'][0]['vendorstoreid'] > 0){

	 	 //echo "<pre>";print_r($cart['item'][0]['vendorstoreid']);exit;
	 	$cart['payment']=Vendorpaymentsettings::editGeneral($cart['item'][0]['vendorstoreid']);
	...
	public function view_checkout(Request $request)
	{
	  ...
	  // echo "</br>";
	   $total_prices = base64_decode($request->input('total_prices'));
	  // echo "</br>";
	  $amount = base64_decode($request->input('amount'));
	 // exit;
	   $sid = 1;
	   ...
	   $setting['setting'] = Settings::editGeneral($sid);
	   $currency=  $setting['setting']->site_currency;
	   $exclsuive_commison=$setting['setting']->site_exclusive_commission;
	   $non_exclusive_commission=$setting['setting']->site_non_exclusive_commission;
	    if($cart['item'][0]['vendorstoreid'] > 0){
	 	...
	 	if(empty($cart['payment'])){
	 		$sid = 1;
			$setting['setting'] = Settings::editGeneral($sid);
			$setting['setting']->site_currency= $currency;
	 	}else{
	 		$setting['setting'] =$cart['payment'];
	 		$setting['setting']->site_currency= $currency;
	 		$setting['setting']->site_exclusive_commission=$exclsuive_commison;
	 		$setting['setting']->site_non_exclusive_commission=$non_exclusive_commission;
	 	}
	   ...
	   $success_url = $website_url.'/psuccess/'.$purchase_token;
	   $cancel_url = $website_url.'/pcancel';
	   ...
	   if($getcount == 0)
	   {
		  ...
		  foreach($order_loop as $order)
		  {
		    $single_order = Products::getorderData($order);
			...
			{
			$price_item = ( $single_order->item_price * $single_order->item_qunatity )- $single_order->coupon_value;
			}
			...
			{
				$price_item =( $single_order->item_price * $single_order->item_qunatity );
			}
			if($buyer_type == 1)
			{
			...
			$item['name'] = Products::singleorderData($order);
		  ...
		  else if($payment_method == 'wallet')
		  {
		      if($buyer_wallet_amount >= $final_amount)
			   {    
					...
					$token = $purchased_token;
					$checking = Products::getcheckoutData($token);
					...
					Members::updateData($user_token,$walet_data); 
					  foreach($order_loop as $order)
					  {
						...
						if($setting['setting']->payment_verification == 0)
						{
							  ...
							  $ordered['data'] = Products::singleorderData($order);
							  $user_id = $ordered['data']->user_id;
							  $item_user_id = $ordered['data']->item_user_id;
							  $vendor_amount = $ordered['data']->vendor_amount;
							  $total_price = $ordered['data']->total_price;
		  ...
		  else if($payment_method == 'stripe')
		  {
				...
				if($chargeResponse['paid'] == 1 && $chargeResponse['captured'] == 1) 
				{
					...
					$token = $purchased_token;
					$check['display'] = Products::getcheckoutData($token);
					$order_id = $check['display']->order_ids;
					$order_loop = explode(',',$order_id);
					  
					foreach($order_loop as $order){
						
						$getitem['item'] = Products::getorderData($order);
						$token = $getitem['item']->item_token;
						$item['display'] = Products::solditemData($token);
						//	$item_sold = $item['display']->item_sold + 1;
						$item_token = $token; 
						//	$data = array('item_sold' => $item_sold);
					 	//   Products::updateitemData($item_token,$data);
