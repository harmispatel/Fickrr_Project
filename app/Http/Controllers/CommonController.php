...
class CommonController extends Controller
{
  ...
  // 		      echo "HTML Email Sent. Check your inbox.";exit;
	...
	// 	$data = array('user_token'=>"Virat Gandhi");
	// 		Mail::send('forgot_mail', ['data' => $data], function($m) use ($data){
	// 		$m->to('harmistest@gmail.com')->subject('Laravel HTML Testing Mail')->getSwiftMessage()
	// 		->getHeaders()
...
return view('user.shop1',['itemData' => $itemData, 'catData' => $catData, 'category' => $category,'location'=>$location,'roomtype'=>$roomtype]);
	...
	public function view_single_item1(Request $request, $item_slug)
	{
	...
	public function view_single_item_product(Request $request, $item_slug, $social_media_type = null){
	...
	$related['items']  = $query->where(function($q){
	 ...
	 if($social_media_type != null){
		// $item_slug, $social_media_type
		$shareItemDetails= Products::with('ratings')->where('item_status','=',1)->where('item_slug',$item_slug)->first();
		$shareItemDetail['url'] = URL::To('/item/'.$item_slug.'/'.$social_media_type);
		// $shareItemDetail['url'] = URL::To('/shopstore/'.$item_slug);
		$shareItemDetail['title'] = $shareItemDetails->item_name ? $shareItemDetails->item_name : 'Product';
		$shareItemDetail['description'] = $shareItemDetails->item_desc ? $shareItemDetails->item_desc : 'Description';
		if($shareItemDetails->item_thumbnail!='')
			$shareItemDetail['image'] = url('/').'/public/storage/items/'.$shareItemDetails->item_thumbnail;
		else
			$shareItemDetail['image'] = url('/').'/public/img/no-image.png';
		$shareItemDetail['shareType'] = $social_media_type;
	}
